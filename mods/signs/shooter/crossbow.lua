...
for _, color in pairs(dye_basecolors) do
	...
	minetest.register_craft({
		output = "shooter:crossbow_loaded_"..color,
                type = "shapeless",
		recipe = {"shooter:crossbow", "shooter:arrow_" .. color},
		})
end
...
if SHOOTER_ENABLE_CRAFTING == true then
	...
	if minetest.get_modpath("creatures") ~= nil then
		...
		recipe = {
			{"default:flint", "", ""},
			{"", "default:stick", "creatures:feather"},
			{"", "creatures:feather", "default:stick"},
		},
	})
		minetest.register_craft({
		output = "shooter:arrow_white 16",
