...
function mob_class:do_attack(player)
	...
	return "attack requested"
end

function mobs:do_attack(entity, player)
	return mob_class.do_attack(entity, player)
end
...
function mob_class:do_stay_near()
	...
	local randpos = nearby_nodes[random(#nearby_nodes)]
	yaw_to_pos(self, randpos)
...
--edited to use ingame days instead of seconds
...
function mob_class:update_tag()
	...
	self.infotext = ""
		...
		self.infotext = self.infotext.."Health: " .. self.health .. " / " .. self.hp_max.. "\n"
		.."Owner: " .. self.owner
		.. text
...
function mob_class:item_drop()
	...
	--if self.food and self.food <= 4 then return end
	local pos = self.object:get_pos()
...
function mob_class:breed()
	...
	if self.child == true then
		...
		if self.hornytimer >= CHILD_GROW_TIME then
			if self.food < 16 or self.water < 16 then
				return
			end
			self.child = false
			self.hornytimer = 0
	...
	and self.hornytimer < HORNY_TIME then
		...
		for n = 1, #objs do
			...
			and ent.hornytimer < HORNY_TIME then
				...
				self.hornytimer = HORNY_TIME
				ent.hornytimer = HORNY_TIME
...
function mob_class:replace(pos)
	...
	if type(self.replace_what[1]) == "table" then
		...
		what = {}
		for i, whattbl in pairs(self.replace_what) do
			table.insert(what, whattbl[1])
		end
	...
	local pos1 = {x=roundpos.x-1, y=roundpos.y-2, z=roundpos.z-1}
	local pos2 = {x=roundpos.x+1, y=roundpos.y+2, z=roundpos.z+1}
	local nodes = minetest.find_nodes_in_area(pos1, pos2, what)
	for i, nodepos in pairs(nodes) do
		if type(what) == "table" then
			local node = minetest.get_node(nodepos)
			for i2, whattbl in pairs(self.replace_what) do
				local tempname = whattbl[1] or ""
				y_offset = self.replace_what[i2][3] or 0
				local temppos = vector.new(nodepos)
				if y_offset >= 1 then
					temppos.y = temppos.y -.5
				else
					temppos.y = temppos.y +.5
				end
				if (node.name == tempname or minetest.get_item_group(node.name, string.gsub(tempname, "group:", "")) > 0)
				and	nodepos.y==(roundpos.y+y_offset)
				--and minetest.line_of_sight(temppos, pos)
				then
					what = self.replace_what[i2][1] or ""
					with = self.replace_what[i2][2] or ""
					break
				end
			end
		end
		if not with or not what then goto next end
-- print("replace node = ".. minetest.get_node(pos).name, pos.y)
		if self.on_replace then
			...
			or oldnode:find("group:") then
				oldnode = minetest.get_node(nodepos).name
			end

			if self:on_replace(nodepos, oldnode, newnode) == false then
				goto next
			end
		end
		minetest.set_node(nodepos, {name = with})
...
function mobs.is_peaceful_player(player)
	return is_peaceful_player(player)
end
...
function mob_class:do_states(dtime)
	...
	elseif self.state == "walk" then
		...
		elseif self.randomly_turn and random(100) <= 30 then
			...
			yaw = yaw + random(-0.5, 0.5)
			if self.herdpos and self.herdpos.x then
				local pos = s
				local fac = 4-vector.distance(pos, self.herdpos)/5
				local herddir = vector.direction(pos, self.herdpos)
				local targetdir = vector.rotate({x=0,y=0,z=fac}, {x=0, y=self.target_yaw, z=0})
				targetdir = vector.add(herddir, targetdir)
				yaw = vector.dir_to_rotation(vector.normalize(targetdir)).y
			end
...
--print ("====", self.hornytimer)
		...
		if self.food and self.food >= feed_count then
