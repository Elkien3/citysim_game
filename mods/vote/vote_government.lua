...
local default_siege_hour = tonumber(minetest.settings:get("default_siege_hour")) or 15
local minimum_siege_hoursfromnow = tonumber(minetest.settings:get("minimum_siege_hoursfromnow")) or 24
...
if areas then
	...
	if areas.siege_create then
		minetest.register_chatcommand("vote_siege", {
			params = "<id> <hoursfromnow> <newowner>",
			description = "Start a vote to start a siege on a protection area, requires owner to vote yes",
			privs = {
				vote_government = true,
			},
			func = function(name, param)
				...
				if not id or not newOwner or not hoursfromnow then
					return false, S("Invalid usage, see"
							.." /help @1.", "vote_siege")
				end
				...
				if not hoursfromnow or not tonumber(hoursfromnow) or tonumber(hoursfromnow) < 0 then return false, "Invalid hours from now" end
				
				local oldOwner = areas.areas[id].owner
				oldOwner = (jobs and (jobs.list[jobs.split(oldOwner, ":")[1]] or {}).ceo) or oldOwner
				
				return vote.new_vote(name, {
					description = "Start siege of area " ..id..
						", to give to "..newOwner,
					help = "/yes,  /no  or  /abstain",
					name = name,
					duration = 15,
					...
					can_vote = function(self, pname)
						return minetest.check_player_privs(pname,{vote_government = true}) or pname == oldOwner
					end,

					on_result = function(self, result, results)
						local yes = results.yes or {}
						local ownervoted = false
						for _, playername in pairs(results.yes) do
							if playername == oldOwner then
								ownervoted = true
								break
							end
						end
						if not ownervoted then
							minetest.chat_send_all(S("Failed to start siege on area @1. Owner did not vote yes, use /vote_siege_default to proceed without owner permission.", id))
						elseif #yes >= votesneeded then
							local currentunixhour = areas.get_unixhour()
							areas.siege_create(id, currentunixhour+math.floor(hoursfromnow), newOwner)
							minetest.chat_send_all(S("A siege on Area @1 will start in @2 hours, new owner will be @3. (@4/@5)", id, hoursfromnow, newOwner, #yes, votesneeded))
						else
							minetest.chat_send_all(S("Failed to start siege on area @1. (@2/@3)", id, #yes, votesneeded))
						end
					end,
		...
		minetest.register_chatcommand("vote_siege_default", {
			params = "<id> <newowner>",
			description = "Start a vote to start a siege on a protection area using default siege time ("..tostring(default_siege_hour).."00 UTC)",
			privs = {
				vote_government = true,
			},
			func = function(name, param)
				...
				if not id or not newOwner then
					return false, S("Invalid usage, see"
							.." /help @1.", "vote_siege_default")
				end
				...
				return vote.new_vote(name, {
					...
					on_result = function(self, result, results)
						...
						if #yes >= votesneeded then
							...
							local hoursfromnow = default_siege_hour-datetbl.hour
							while hoursfromnow < minimum_siege_hoursfromnow do
		...
		minetest.register_chatcommand("vote_end_siege", {
			params = "<id>",
			description = "Start a vote to end the siege on a protection area",
			privs = {
				vote_government = true,
			},
			func = function(name, param)
				...
				if not id then
					return false, S("Invalid usage, see"
							.." /help @1.", "vote_end_siege")
				end
				
				if not areas.siege_get(id) then
					return false, S("No siege on area.")
				end
				
				return vote.new_vote(name, {
					description = "Cancel siege of area " ..id,
					help = "/yes,  /no  or  /abstain",
					name = name,
					duration = 15,
					perc_needed = 0,
					...
					on_result = function(self, result, results)
						...
						if #yes >= votesneeded then
							...
							minetest.chat_send_all(S("Ended siege on Area @1 (@2/@3)", id, #yes, votesneeded))
						else
							minetest.chat_send_all(S("Failed to end siege on area @1. (@2/@3)", id, #yes, votesneeded))
						end
					end,
