...
--  DESTROY BOBBER WHEN PUNCHING IT
	...
	on_activate = function(self, staticdata, dtime_s)
		...
		self.randomtime = fishing_setting.get_fish_time(pos, math.random(4, 8))
	end,
		...
		if string.find(item_name, "fishing:pole_") ~= nil then
			...
			if self.prize ~= "" then
				if math.random(1, 100) <= fishing_setting.settings["escape_chance"] then -- fish escaped
					...
					local itemstack = ItemStack({name=name, count=1, wear=wear_value, metadata=""})
					local expiration = minetest.registered_items[name].expiration
					if expiration then
						local meta = itemstack:get_meta()
						local newexpiration = minetest.get_day_count() + expiration
					...
					if inv:room_for_item("main", itemstack) then
						...
						minetest.spawn_item(clicker:get_pos(), itemstack)
					end
					fishing_setting.take_fish(pos)
				end
			else
				if not fishing_setting.is_creative_mode then
					if inv:room_for_item("main", {name=self.bait, count=1, wear=0, metadata=""}) then
...
-- AS SOON AS THE BOBBER IS PLACED IT WILL ACT LIKE
	on_step = function(self, dtime)
		...
		if not node or (minetest.get_modpath("static_ocean") and node.name ~= "static_ocean:water_source") or string.find(node.name, "water_source") == nil then
			if fishing_setting.settings["message"] == true then
				if minetest.get_modpath("static_ocean") then
					minetest.chat_send_player(self.owner, fishing_setting.func.S("Fishing is prohibited outside natural water!"))
				else
					minetest.chat_send_player(self.owner, fishing_setting.func.S("Fishing is prohibited outside water!"))
				end
			end
			self.object:remove()
			return
		...
		--minetest.chat_send_all(self.timer)
		--minetest.chat_send_all(self.randomtime)
		...
		if math.random(1, 100) > fishing_setting.baits[self.bait]["hungry"] + self.baitball then
			--Fish not hungry !(
			self.randomtime = fishing_setting.get_fish_time(pos, math.random(20,60))
			return
		end

		self.randomtime = math.random(1,5)
				...
				for _, obj in pairs(objs) do
					if obj:get_luaentity() ~= nil then
						...
						if fishing_setting.prizes["true_fish"]["little"][name] then
							...
							obj:remove()
							self.randomtime = math.random(3,7)
							break
						end
					end
				end
