...
local function update_fence(pos)
    if minetest.get_node(pos).name:find("army:chainlink") == nil then
        return
    end
    local sum = 0
    for i = 1, 4 do
    ...
    minetest.add_node(pos, {name = "army:chainlink_"..sum})
end
...
for i = 1, 15 do
    ...
    local texture = "army_chainlink.png"
    if cnt == 1 then
        texture = "army_chainlink.png"
    end
...
minetest.register_node("army:chainlink", {
    description = "Chainlink Fence",
    tiles = {"army_chainlink.png"},
